...
function isHtmlLike(input: string) {
...
function splitStructuredLine(input: string) {
...
function extractLinesFromHtml(input: string) {
...
function looksLikeMarkdown(line: string) {
  return /^(#{1,6}\s|\-\s|\*\s|\d+\.\s|>\s|```|\|.+\|$)/.test(line);
}

function isMainTitle(line: string, index: number) {
  if (index !== 0) {
    return false;
  }
  if (line.length > 40) {
...
function toMarkdownLine(line: string, index: number) {
  if (!line) {
    return "";
  }
...
export function normalizeToMarkdown(input: string) {
