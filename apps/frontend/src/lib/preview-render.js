...
export function normalizeImageMarkup(html) {
...
export function preprocessMarkdownSource(source) {
    return (source ?? "")
        .replace(/\r/g, "")
        .replace(/^(>\s+)(\([a-zA-Z0-9]+\)|（[一二三四五六七八九十]+）|[a-zA-Z0-9]+\.)/gm, "$2");
}
export function normalizePreviewMarkup(html) {
    return html
...
export function renderMarkdownPreviewFragment(source) {
    return normalizePreviewMarkup(normalizeImageMarkup(marked.parse(preprocessMarkdownSource(source ?? ""))));
}
export function ensureGuestHtml(html) {
    if (!html.trim()) {
        return "";
    }
    return html
